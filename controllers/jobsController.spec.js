...
describe('Jobs Controller', () => {
  ...
  describe('Get single Job', () => {
    it('should throw job not found error', async () => {
      ...
      const mockReq = mockRequest().params = {
          params: { id: "6453a46d8379ab23e4d6bbbd" }
      };
  ...
  describe('Should get single Job by id', () => {
    it('should throw job not found error', async () => {
    ...
    it('should throw invalid ID error', async () => {
      ...
      const mockReq = (mockRequest().params = {
        params: { id: '6453a46d8379ab23e4d5bbbd' }
      });
      ...
      expect(mockRes.status).toHaveBeenCalledWith(400);
      expect(mockRes.json).toHaveBeenCalledWith({  error: "Please enter correct id" })
    });
  });

  describe('Update a job', () => {
    it('should job not found error', async () => {
      jest.spyOn(Job, 'findById').mockResolvedValueOnce(null);
    ...
    it('It should update the job', async () => {
      ...
      const updatedJob = Object.assign(mockJob, { title: "Senior Node Developer II" });
      jest.spyOn(Job, "findByIdAndUpdate").mockResolvedValueOnce(updatedJob);
      const mockReq = mockRequest().params = {
        params: { id: "6453a46d8379ab23e4d6bbbd" },
        user: { id: '6453a46d8379ab23e4d5bbbd' },
        body: { title: "Senior Node Developer II" }
      };
      ...
      await updateJob(mockReq, mockRes);
      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(mockRes.json).toHaveBeenCalledWith({ job: updatedJob });
    });
  });
